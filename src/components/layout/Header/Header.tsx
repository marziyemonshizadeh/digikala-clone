...
function Header() {
  ...
  return (
    <header className="flex-col top-0 z-40 w-full select-none sticky border-b border-gray-300 bg-white px-4 shadow">
      <div className="lg:flex block lg:justify-between items-center py-3">
        ...
        <div className="lg:flex hidden items-center gap-3">
          ...
          <div className="border-s border-[#E0E0E2] ps-5">
            <button
              className=" px-1 relative border-2 border-transparent text-gray-800 rounded-full hover:text-gray-500 focus:outline-none focus:text-gray-500 transition duration-150 ease-in-out"
              aria-label="Cart"
            >
              <CartOff />
              {productCount && (
                <span className="absolute inset-0 top-1 object-right-bottom -mr-6">
      ...
      {/* navbar */}
      <nav
        className="lg:flex hidden justify-between font-medium text-sm"
        style={{
          height: " 40px",
          // padding: "0px 16px",
        }}
      >
        <div className="flex justify-between gap-4">
          ...
          <Link href={"incredible-offers"} className="flex items-center text-gray-500 gap-2 px-2 border-b-[3px] border-transparent hover:border-[#FF0800] transition-border duration-150">
            <LuBadgePercent />
            <span>شگفت انگیزها</span>
          </Link>
          <Link href={"/fresh"} className="flex items-center text-gray-500 gap-2 px-2 border-b-[3px] border-transparent hover:border-[#FF0800] transition-border duration-150">
            <LuBadgePercent />
            <span>سوپرمارکت</span>
          </Link>
          <Link href={"/best-selling"} className="flex items-center text-gray-500 gap-2 px-2 border-b-[3px] border-transparent hover:border-[#FF0800] transition-border duration-150">
            <LuBadgePercent />
            <span>پرفروش ترین ها</span>
          </Link>
          <Link href={"/faq"} className="flex items-center text-gray-500 gap-2 px-2 border-b-[3px] border-transparent hover:border-[#FF0800] transition-border duration-150">
            <LuBadgePercent />
            <span>سوالی دارید؟</span>
          </Link>
          <Link href={"/landings"} className="flex items-center text-gray-500 gap-2 px-2 border-b-[3px] border-transparent hover:border-[#FF0800] transition-border duration-150">
            <LuBadgePercent />
